...
try {
    ...
    if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
        ...
        $stmtDelete = $pdo->prepare("DELETE FROM arh WHERE id = ?");
        $stmtDelete->execute([$deleteId]);
        // Перенаправляем, чтобы избежать повторного удаления при обновлении страницы
        header("Location: showDel.php");
        exit;
    }
...
foreach ($rows as $row) {
    ...
    echo "<td><a href=\"?delete=" . $row['id'] . "\" onclick=\"return confirm('Вы уверены, что хотите удалить запись?');\">Удалить</a></td>\n";
    echo "</tr>\n";
}
?>
